...
# - Digest alerts are sourced from $WORKDIR/state/YYYY-MM-DD.candidates.json.
...
python3 - "$WWW_DIR/status.json" "$REPORT_TXT" "$CANDIDATES_JSON" "$WWW_DIR/index.html" <<'PY'
import html, json, os, sys, hashlib

status_path, report_path, candidates_path, out_path = sys.argv[1:5]
...
# Flatten candidates into "alerts".
alerts = []
if candidates:
