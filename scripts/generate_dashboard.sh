...
# Render daily page (digest + feedback UI + raw report).
# Digest data is expected at: $WORKDIR/state/YYYY-MM-DD.candidates.json (generated by run_daily.sh).
CANDIDATES_JSON="$WORKDIR/state/${TODAY_ET}.candidates.json"

python3 - "$REPORT_TXT" "$CANDIDATES_JSON" "$DAILY_DIR/${TODAY_ET}.html" "$TODAY_ET" <<'PY'
import html, json, os, sys, hashlib

report_path, candidates_path, out_path, day = sys.argv[1:5]
...
# Build a flat list of "digest alerts" from candidates.json (best-effort).
alerts = []
if candidates:
    for item in candidates.get('new_external_destinations', []) or []:
        alerts.append({
            'kind': 'new_external_destination',
            'title': f"New external destination: {item.get('dst_ip','?')}",
            'data': item,
        })
    for d in candidates.get('new_domains', []) or []:
        alerts.append({
            'kind': 'new_domain',
            'title': f"New domain: {d}",
            'data': {'domain': d},
        })
    for item in candidates.get('new_watch_tuples', []) or []:
        alerts.append({
            'kind': 'new_watch_tuple',
            'title': f"New watch tuple: {item.get('src_ip','?')} → {item.get('dst_ip','?')}:{item.get('dst_port','?')}",
            'data': item,
        })
    for b in candidates.get('rita_beacons', []) or []:
        alerts.append({
            'kind': 'rita_beacon',
            'title': f"RITA beacon candidate: {b.get('src_ip','?')} → {b.get('dst_ip','?')}",
            'data': b,
        })
...
body.append('<html><head><meta charset="utf-8">')
body.append('<meta name="viewport" content="width=device-width,initial-scale=1">')
body.append(f'<title>HomeNetSec — Digest — {html.escape(day)}</title>')
body.append(f'<style>{style}</style>')
body.append('</head>')
body.append(f'<body data-day="{html.escape(day)}">')
body.append(f'<h1>HomeNetSec — Daily Digest</h1>')
body.append(f'<div class="muted">{html.escape(day)}</div>')
body.append('<p><a href="/">← Dashboard</a> · <a href="/daily/">All days</a></p>')

body.append('<div class="card">')
body.append('<h2>Digest alerts (for review)</h2>')
body.append('<div class="muted">This page is structured for analyst-style triage. Feedback is stored in your browser (localStorage) for now; use “Export feedback” to share/apply.</div>')
body.append('<p><button onclick="exportFeedback()">Export feedback JSON</button></p>')

if not alerts:
    body.append('<div class="muted">No candidates file found (or no alerts). Once the daily pipeline runs, this will populate from <code>output/state/YYYY-MM-DD.candidates.json</code>.</div>')
else:
    for a in alerts:
        ...
        body.append(f'<div style="margin-top:10px"><label>Comment / context:<br><textarea id="note-{aid}" placeholder="e.g. This is the doorbell doing NTP lookups (pool.ntp.org)."></textarea></label></div>')
...
body.append('<div class="card">')
body.append('<h2>Raw daily roll-up (current)</h2>')
if report_txt.strip():
...
body.append(f'<script id="candidates" type="application/json">{html.escape(json.dumps(payload))}</script>')
